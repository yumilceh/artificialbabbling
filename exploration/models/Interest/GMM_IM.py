...
class GMM_IM(object):
    ...
    def get_goal_proprioception(self, agent, sm_model, ss_model):
    ...
    def sortInterestingGaussians(self, active_comp=None):
        ...
        for k, (Covar, Mean) in enumerate(zip(gmm._get_covars(), gmm.means_)):
            if (Covar[time_index, competence_index] > 0):
                ...
                cov_interesting_gauss[n_interesting_gauss] = Covar[
                    competence_index, time_index];  # abs(sum(GMM_IM.Sigma(1:6,7,k)));    %based on covariance respect to time/ covariance respect to auditory goals7
