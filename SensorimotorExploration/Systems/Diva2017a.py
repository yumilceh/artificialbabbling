...
class Diva2017a(object):
    def __init__(self):
        ...
        SOMATO_NEW_names = ['SS1', 'SS2', 'SS3', 'SS4', 'SS5', 'SS6', 'SS7', 'SS8', 'SS9', 'SS10', 'SS11', 'SS12', 'SS13', 'SS14', 'SS15',
                       'SS16']
        somato_names = ['P1']
        ...
        min_SOMATO_NEW_values = np.array([-1.0] *n_SOMATO_NEW)
        max_SOMATO_NEW_values = np.array([1.0] *n_SOMATO_NEW)
        ...
        self.n_SOMATO_NEW = n_SOMATO_NEW
        self.n_somato = n_somato

        self.motor_names = motor_names
        self.sensor_names = sensor_names
        self.SOMATO_NEW_names = SOMATO_NEW_names
        ...
        self.min_SOMATO_NEW_values = min_SOMATO_NEW_values
        self.max_SOMATO_NEW_values = max_SOMATO_NEW_values
        ...
        self.SOMATO_NEW_out = np.array([0.0] * n_SOMATO_NEW)
        self.SOMATO_NEW_goal = np.array([0.0] * n_SOMATO_NEW)
        self.somato_out = 0.0 #Constraint violations flag
        self.somato_threshold = 0.5
    ...
    def vocalize(self):
        ...
        auditory_states, SOMATO_NEW_states, tr__, af = self.synth.get_audsom(self.art_states[:,0:13],scale=True)
        self.auditory_states = auditory_states
        self.SOMATO_NEW_states = SOMATO_NEW_states
        self.af = af
        minaf = np.array([np.min(x) for x in af])
        self.somato_states = minaf

        self.sensor_out = 0.0 * self.sensor_out
        self.SOMATO_NEW_out = 0.0 * self.SOMATO_NEW_out
        ...
        for index in range(n_percep_samples):
            ...
            for jj in range(8):
                self.SOMATO_NEW_out[jj] += SOMATO_NEW_states[index+2, jj]/n_percep_samples
            somato_av[0] = somato_av[0] + (minaf[index + 2] / n_percep_samples)
        ...
        for index in range(n_percep_samples):
            ...
            for jj in range(8):
                self.SOMATO_NEW_out[jj+8] += SOMATO_NEW_states[index + 43, jj] / n_percep_samples
            somato_av[1] = somato_av[1] + (minaf[index + 43] / n_percep_samples)
        ...
        self.somato_out = 0.0
        if ((somato_av[0] < 0.0) or (somato_av[1] < 0.0)):
            self.somato_out = 1.0
    ...
    def plot_som_out(self, som_idx, axes=None):
        ...
        for index in range(len(som_idx)):
            plt.plot(self.time, self.SOMATO_NEW_states[:, som_idx[index]])

    def plot_somato_out(self, axes=None):
        if axes is None:
            fig, axes = plt.subplots(1,1)
        plt.sca(axes)
        plt.plot(self.time, self.somato_states)
