...
class ExplautoSS(object):
    ...
    def __init__(self, system, model_type='nearest_neighbor', model_conf ="default"):
        ...
        self.set_sigma_explo(0.)  # For somatosensory data we are not interested on exploratory noise
        self.params = PARAMS()
        self.params.ss_step=1 #only ok with non-parametric
       
    def predict_somato(self, system, motor_command=None):
        if motor_command is None:
            motor_command = system.motor_command  #s_g
        
        system.somato_prediction = self.model.forward_prediction(motor_command)
